document.addEventListener("DOMContentLoaded", () => {
  ...
  if (homeGallery) {
    ...
    populateHomeGallery(); // Initial population

    const shuffleBtn = document.getElementById('shuffleHomeBtn');
    if (shuffleBtn) {
      ...
      const startAutoShuffle = () => {
        clearInterval(homeAutoShuffle);
        if (!isHovering) {
          homeAutoShuffle = setInterval(() => { shuffleBtn.click(); }, 4000);
        }
      };
      ...
      shuffleBtn.addEventListener('click', () => {
        ...
        const cards = homeGallery.querySelectorAll('.image-card');
      ...
      startAutoShuffle();
      homeGallery.addEventListener('mouseenter', () => {
        isHovering = true;
        clearInterval(homeAutoShuffle);
      });
      homeGallery.addEventListener('mouseleave', () => {
        isHovering = false;
        startAutoShuffle();
      });
    }
  ...
  if (memberId && memberName) {
    ...
    if (dynamicGallery) {
      let startIdx = (parseInt(memberId, 10) * 11) % imgPool.length;
      if (isNaN(startIdx)) startIdx = 0;
      
      for (let i = 0; i < 6; i++) {
        const candidIndex = (startIdx + i * 5) % imgPool.length;
        const imgName = imgPool[candidIndex];
        
        const div = document.createElement("div");
        div.className = "image-card";
        const encodedName = encodeURIComponent(imgName);
        div.innerHTML = `<img src="https://raw.githubusercontent.com/noemaann/Zlu-images/main/images/${encodedName}" alt="${memberName} photo ${i + 1}">`;
